...
export default function PortalAssignments() {
  ...
  const load = () => {
    Promise.all([getAssignments(session.userId), getSubmissions(session.userId)]).then(
      ([a, s]) => {
        setAssignments(a.sort((x, y) => +new Date(x.deadline) - +new Date(y.deadline)));
        setSubmissions(s);
      },
    );
  };
                  ...
                  <td>
                    {sub ? (
                      <>
                        {sub.fileName}
                        <br />
                        <small>{new Date(sub.submittedAt).toLocaleString("en-AU")}</small>
                      </>
                    ) : (
                      "—"
                    )}
        ...
        <div className="portal-panel">
          ...
          >
            <div className="form-row">
              <label htmlFor="assignment-file">Select file (PDF, DOC, ZIP)</label>
              <input
                id="assignment-file"
                type="file"
